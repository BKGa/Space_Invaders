...
BLACK = (0, 0, 0)
GREY = (130, 130, 130)
LIGHT_GREEN = (66, 245, 144)


class LandingPage:
    alien_one_imgs = [pg.image.load(f'images/greenalien{n}.png') for n in range(3)]
    alien_two_imgs = [pg.image.load(f'images/purplealien{n}.png') for n in range(3)]
    alien_three_imgs = [pg.image.load(f'images/angryalien{n}.png') for n in range(4)]
    ufo_imgs = [pg.image.load(f'images/ufo{n}.png') for n in range(4)]
    ...
    def __init__(self, game):
        ...
        strings = [('SPACE', WHITE, headingFont), ('INVADERS', WHITE, subheadingFont),
               ...
               # ('PLAY GAME', GREEN, font),
                (f'HIGH SCORES = {self.highscore:,}', GREY, font)]
        ...
        self.alien_one = Alien(game=game, sound=self.sound, alien_index=0, image_list=LandingPage.alien_three_imgs,
        ...
        self.alien_three = Alien(game=game, sound=self.sound, alien_index=2, image_list=LandingPage.alien_one_imgs,
                               v=Vector(), ul=(centerx - 120, 490))
        ...
        for e in pg.event.get():
            ...
            if e.type == pg.KEYUP and e.key == pg.K_p:   # pretend PLAY BUTTON pressed
                ...
                if self.play_button.rect.collidepoint(mouse_x, mouse_y):
                    self.landing_page_finished = True
            if self.play_button.rect.collidepoint(mouse_x, mouse_y):
                self.play_button.color = LIGHT_GREEN
                self.play_button.text_color = LIGHT_GREEN
            else:
